...
> **Note**: the IR (intermediary representation) that PJRT plugin accepts are "HLO" or "StableHLO".
> They not usually written directly by humans (except for debugging/developing the insides of PJRT),
> and instead are generated by, for instance, a Jax/PyTorch/Tensorflow program, or using the `xlabuilder`
