...
func (dtype DType) PrimitiveType() protos.PrimitiveType {
	switch (dtype) {
	case InvalidDType:
		return protos.PrimitiveType_PRIMITIVE_TYPE_INVALID{{range .}}{{if .HasPrimitiveType}}
	case {{.Name}}:
		return protos.PrimitiveType_{{.Original}}{{end}}{{end}}
	default:
...
func FromPrimitiveType(primitiveType protos.PrimitiveType) DType {
	...
	case protos.PrimitiveType_{{.Original}}:
		return {{.Name}}{{end}}{{end}}
	default:
		return InvalidDType
	}
...
func generateEnums(contents string) {
	...
	for _, line := range strings.Split(matches[2], "\n") {
		...
		// Find PrimitiveType:
		_, enumV.HasPrimitiveType = protos.PrimitiveType_value[enumV.Original]
