...
func (dtype DType) PrimitiveType() proto.PrimitiveType {
	switch (dtype) {
	case InvalidDType:
		return proto.PrimitiveType_PRIMITIVE_TYPE_INVALID{{range .}}{{if .HasPrimitiveType}}
	case {{.Name}}:
		return proto.PrimitiveType_{{.Original}}{{end}}{{end}}
	default:
...
func FromPrimitiveType(primitiveType proto.PrimitiveType) DType {
	...
	case proto.PrimitiveType_{{.Original}}:
		return {{.Name}}{{end}}{{end}}
	default:
		return InvalidDType
	}
...
func generateEnums(contents string) {
	...
	for _, line := range strings.Split(matches[2], "\n") {
		...
		// Find PrimitiveType:
		_, enumV.HasPrimitiveType = proto.PrimitiveType_value[enumV.Original]
